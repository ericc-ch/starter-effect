...
- **Key Deliverables**:
  ...
  - [ ] **CORS Middleware**: Create `apps/api/src/lib/http/cors.ts` using `HttpMiddleware.cors` from `@effect/platform` with origin from `EnvContext`.
...
- **Key Deliverables**:
  - [ ] **Root RPC Group**: Create `apps/api/src/rpc/main.ts` that combines all RPC groups (currently just `BooksRpcGroup`) into a single `RpcGroup`.
...
- **Key Deliverables**:
  - [ ] **Switch Entry**: Rename `apps/api/src/main_effect.ts` to `apps/api/src/main.ts`, backup old main.
